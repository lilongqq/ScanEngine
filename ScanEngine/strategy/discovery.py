...
from crawling.checklist import Checklist
from crawling.diff import Differator
from crawling.extractcrawler import ExtractIterator
...
class Discovery(Strategy):
    ...
    def check_status(self):
        ...
        if status in {None, b'stop'}:
            ...
            binary_count = self.redis.hget(self.stats_name, 'binary_count')
            self.binary_count = int(binary_count) if binary_count else 0
    ...
    def run(self):
        ...
        count, db_records, binary_count, size = self.redis.hmget(
            self.stats_name,
            ['count', 'db_records', 'binary_count', 'size']
        )
        if count:
        ...
        if binary_count:
            self.binary_count = int(binary_count)
        if size:
            self.size = int(size)
        self.logger.info(
            '''
    ...
    def logging_error(self, node, future):
        ...
        if exception:
            ...
            if node.get('type') == 'binary':
                pipeline.hincrby(self.stats_name, 'binary_count')
            pipeline.execute()
            return
