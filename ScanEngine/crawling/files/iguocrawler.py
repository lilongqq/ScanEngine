...
class IguoIterator(Iterator):
    ...
    def __next__(self):
        while True:
            if self.file_msg and self.file_msg['content']['file_msg']:
                ...
                node['auth'] = self.auth
                node['cls'] = 'IGUO'
                return node, partial(self.get_file, node)
                ...
                if msg_type == 2:
                    ...
                    if node.get('name', ''):
                        ...
                        node['name'] = node['msg_id']
                        node['path'] = node['msg_id']
                    node['feature_id'] = self.node['feature_id']
                    node['last_write_time'] = node['send_time']
                    return node, partial(self.wrapper_logs, node)
    ...
    @staticmethod
    def wrapper_logs(node):
        content = node.pop('content')
        data = json.dumps(content, ensure_ascii=False).encode(encoding='utf-8')
        node['size'] = len(data)
        md5 = hashlib.md5()
        md5.update(data)
