...
class EWS(Client):
    ...
    def __init__(
        ...
        if self.ssl_wrap:
            self.service_endpoint = f'https://{self.server}:{self.port}/EWS/Exchange.asmx'
            if self.ews_version == 'EXCHANGE_2019':
                BaseProtocol.HTTP_ADAPTER_CLS = NoVerifyHTTPAdapter
            else:
                BaseProtocol.HTTP_ADAPTER_CLS = NoVerifyHTTPAdapterTLSv1
        else:
            self.service_endpoint = f'http://{self.server}:{self.port}/EWS/Exchange.asmx'
    ...
    def get_nodes(self, node={}):
        nodes = list()
        if node and node.get('folder_class'):
            path = node.get('path')
